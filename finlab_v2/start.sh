...
(
  ...
  /app/.venv/bin/python ui_smoke.py || echo TRIAID_UI_NONBLOCKING_AUDIT_FAILED
) &
(
  sleep "${TRIAID_POST_START_LIVE_BOOTSTRAP_DELAY:-20}"
  /app/.venv/bin/python hk_market_live_bootstrap.py || echo TRIAID_HK_MARKET_LIVE_FAILED
) &
(
  sleep "${TRIAID_POST_START_LIVE_BOOTSTRAP_DELAY:-20}"
  /app/.venv/bin/python policy_hazard_live_bootstrap.py || echo TRIAID_POLICY_HAZARD_LIVE_FAILED
) &
(
  sleep "${TRIAID_POST_START_FULL_AUDIT_DELAY:-60}"
  /app/.venv/bin/python risk_center_full_audit.py || echo TRIAID_RISK_CENTER_FULL_AUDIT_FAILED
) &

exec /app/.venv/bin/uvicorn app:app --host 0.0.0.0 --port "${PORT:-8080}"
