...
<div class="wrap">
  ...
  <div class="prospective-panel" id="usReturnMaxPanel">
    ...
    <div class="tablewrap" style="max-height:330px">
      <table>
        <thead><tr>
          <th id="usrmThStrategy">策略</th>
          <th id="usrmThWeight">Return-Max 权重</th>
          <th id="usrmThGeneric">通用 Core 权重</th>
        </tr></thead>
    ...
    <div class="tablewrap" style="max-height:280px">
      <table>
        <thead><tr>
          <th id="usrmAhAsset">ETF</th>
          <th id="usrmAhWeight">目标权重</th>
        </tr></thead>
    ...
    <div class="tablewrap" style="max-height:330px">
      <table>
        <thead><tr>
          <th id="usrmChCapital">起始资金</th>
          <th id="usrmChInvested">目标投入</th>
          <th id="usrmChParticipation">单日ADV占比</th>
          <th id="usrmChDays">最少成交天数</th>
          <th id="usrmChCost">预计往返成本</th>
        </tr></thead>
    ...
    <div class="tablewrap" style="max-height:360px">
      <table>
        <thead><tr>
          <th id="usrmRhCapital">起始资金</th>
          <th id="usrmRhFill">成交比例</th>
          <th id="usrmRhEquity">当前净值</th>
          <th id="usrmRhPnl">净利润</th>
          <th id="usrmRhReturn">净收益率</th>
          <th id="usrmRhCost">执行成本</th>
        </tr></thead>
    ...
    <div class="tablewrap" style="max-height:330px">
      <table>
        <thead><tr>
          <th id="usrmDhDate">结果日</th>
          <th id="usrmDhRoute">Return-Max 累计</th>
          <th id="usrmDhGeneric">通用 Core 累计</th>
          <th id="usrmDhSpy">SPY 累计</th>
        </tr></thead>
...
function fmtUsd(x){if(x===null||x===undefined)return '-';const v=Number(x);if(!Number.isFinite(v))return '-';return 'function signedPct(x){if(x===null||x===undefined)return '-';const v=100*x;return (v>0?'+':'')+v.toFixed(2)+'%'}
...
function renderUSReturnMax(report){
 ...
 el('usrmAssetRows').innerHTML=Object.entries(aw).filter(([_,w])=>Number(w)>1e-12).sort((a,b)=>Number(b[1])-Number(a[1])).map(([a,w])=>
  '<tr><td>'+esc(a)+'</td><td class="num">'+fmtPct(w)+'</td></tr>'
 ).join('') || '<tr><td colspan="2">'+(lang==='zh'?'当前无风险ETF敞口':'No current risky ETF exposure')+'</td></tr>';
 const cap=d.capital_capacity||{};
 el('usrmCapitalMeta').textContent=(lang==='zh'?'冻结执行参数：':'Frozen execution parameters: ')+
  'ADV20 · '+fmtPct(cap.max_participation_adv)+' cap · '+(cap.base_cost_bps??'-')+'bps base · '+(cap.impact_coefficient_bps??'-')+'bps×√participation';
 el('usrmCapitalRows').innerHTML=(cap.sleeves||[]).map(x=>'<tr>'+
  '<td class="num">'+fmtUsd(x.starting_capital_usd)+'</td>'+
  '<td class="num">'+fmtUsd(x.target_invested_notional_usd)+'</td>'+
  '<td class="num">'+fmtPct(x.max_one_day_participation_adv)+'</td>'+
  '<td class="num">'+esc(x.minimum_execution_days??'-')+'</td>'+
  '<td class="num">'+fmtUsd(x.estimated_round_trip_cost_proxy_usd)+'</td></tr>'
 ).join('') || '<tr><td colspan="5">'+(lang==='zh'?'等待资金容量决策':'Awaiting capacity decision')+'</td></tr>';
 const rs=((review&&review.capital_sleeves)||{}).sleeves||[];
 el('usrmRealizedRows').innerHTML=rs.map(x=>'<tr>'+
  '<td class="num">'+fmtUsd(x.starting_capital_usd)+'</td>'+
  '<td class="num">'+fmtPct(x.fill_ratio)+'</td>'+
  '<td class="num">'+fmtUsd(x.current_equity_usd)+'</td>'+
  '<td class="num '+cls(Number(x.current_net_pnl_usd||0))+'">'+fmtUsd(x.current_net_pnl_usd)+'</td>'+
  '<td class="num '+cls(Number(x.current_net_return||0))+'">'+signedPct(x.current_net_return)+'</td>'+
  '<td class="num">'+fmtUsd(x.total_execution_cost_usd)+'</td></tr>'
 ).join('') || '<tr><td colspan="6">'+(lang==='zh'?'上一轮尚无真实执行结果':'No realized execution result for the prior decision yet')+'</td></tr>';
 const path=(review&&review.daily_path)||[];
 el('usrmDailyRows').innerHTML=path.map(x=>'<tr>'+
  '<td class="nowrap">'+esc(x.as_of||'-')+'</td>'+
  '<td class="num '+cls(Number(x.return_max_cumulative_return||0))+'">'+fmtPct(x.return_max_cumulative_return)+'</td>'+
  '<td class="num '+cls(Number(x.generic_core_cumulative_return||0))+'">'+fmtPct(x.generic_core_cumulative_return)+'</td>'+
  '<td class="num '+cls(Number(x.spy_buy_hold_cumulative_return||0))+'">'+fmtPct(x.spy_buy_hold_cumulative_return)+'</td></tr>'
 ).join('') || '<tr><td colspan="4">'+(lang==='zh'?'等待下一完整美股交易日结果':'Awaiting the next complete US trading-day outcome')+'</td></tr>';
}
