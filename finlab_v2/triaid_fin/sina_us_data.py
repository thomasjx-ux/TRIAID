...
class SinaUSMarketDataProvider:
    ...
    def _get_payload(self,url:str,params:dict,timeout:int):
        ...
        string_match=re.search(r'=\(?["\'](.*?)["\']\)?\s*;?        if left>=0 and right>left:
    ...
    def _one_minute(self,symbol:str,min_points:int,timeout:int):
        ...
        if isinstance(payload,str):
            # Sina US minute-line format:
            # "09:30:00,volume,price,avg;09:31:00,volume,price,avg;..."
            # The date is omitted, so anchor it to the latest daily bar date.
            daily=self._daily(symbol,2,timeout)
            anchor=datetime.fromtimestamp(daily[-1][0],tz=timezone.utc).date().isoformat()
            rows=[]
            previous_volume=0.0
            for item in payload.split(";"):
                ...
                try:
                    hhmm=cols[0].strip()[:5]
                    cumulative=float(cols[1] or 0.0)
                    price=float(cols[2])
                    stamp=self._minute_ts(f"{anchor} {hhmm}")
                    bar_volume=max(0.0,cumulative-previous_volume)
                    previous_volume=max(previous_volume,cumulative)
                except Exception:
                    continue
                if price>0 and math.isfinite(price):
                    rows.append((stamp,price,bar_volume))
        else:
            rows=self._normalize_rows(payload,daily=False)
        if len(rows)<min_points:
