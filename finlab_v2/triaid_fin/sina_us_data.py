...
class SinaUSMarketDataProvider:
    ...
    version="sina-us-market-data@0.1.1"
    ...
    def _get_payload(self,url:str,params:dict,timeout:int):
        ...
        # US_MinlineNService returns a JavaScript string:
        # var triaid=("09:30:00,...;09:31:00,...")
        for marker,closer in (( '=("', '")' ), ("=('", "')"), ('="','"')):
            start=raw.find(marker)
            if start>=0:
                end=raw.rfind(closer)
                if end>start:
                    return raw[start+len(marker):end]
        ...
        for candidate in candidates:
            try:
                ...
                continue
        raise SinaUSDataError(f"jsonp_parse_failed:{raw[:160]}")
