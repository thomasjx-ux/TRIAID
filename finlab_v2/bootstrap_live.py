...
def require_bootstrap_storage_isolation()->None:
    backend=(os.getenv("TRIAID_STORAGE_BACKEND","file").strip().lower() or "file")
    allow_persistent=os.getenv("TRIAID_BOOTSTRAP_ALLOW_PERSISTENT","0").strip()=="1"
    if backend!="file" and not allow_persistent:
        raise RuntimeError(
            "bootstrap_live refuses non-file storage by default because bootstrap "
            "can create official evidence. Use an isolated file backend for "
            "deployment validation. Set TRIAID_BOOTSTRAP_ALLOW_PERSISTENT=1 only "
            "for an explicit one-time migration."
        )
...
def main()->None:
    ...
    for market_id in ("US","CN"):
        ...
        if existing is None or needs_recovery_bootstrap or needs_us_return_bootstrap:
            run=engine.create_pending_live_run(market_id,"OFFICIAL_EVIDENCE")
            engine.execute_live(run.run_id,market_id,"OFFICIAL_EVIDENCE")
